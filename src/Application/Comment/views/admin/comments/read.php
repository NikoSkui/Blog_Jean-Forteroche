...
<section class="section">
  <?php foreach ($elements[0] as $k => $comment):?>
    ...
    <?php if (isset($comment->indexBook) && $comment->index === $comment->indexBook): ?>
      <div class="section">
        <article class="media">
          <div class="media-content parent">
    ...
    <?php if (isset($comment->indexChapter) && $comment->index === $comment->indexChapter): ?>
      ...
      <article class="media" style="margin-left:30px">
        <div class="media-content">
          <div class="content chapter" id="comment-<?= $comment->id ?>"
          >
    ...
    <!--Insertion Comment  -->
    <?= $renderer->render('@comment/admin/comments/comment', compact('indexEndChapter','comment','elements','index', 'comment->indexChapter','indexComment')) ?>
    ...
    <?php if (isset($comment->indexEndChapter) && $comment->indexEndChapter === -1): ?>
          </div>
        </article>
      </div>
    <?php endif ?>
